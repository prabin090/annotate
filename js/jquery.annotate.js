...
//        this.canvas.children('.image-annotate-view').hide();
        ...
        if (this.editable) {
            ...
            //For Arrow
            this.button_arrow.click(function () {

                var note = {arrowHeadLength: 10, x1: 80, y1: 5, x2: 100, y2: 15}
                var editable = new $.fn.annotateEdit_arrow(image, note);
//                $.fn.annotateImage.add(image);
            });
...
//        if (image.mode == 'view') {
        ...
        var note = {id: "new", top: 0, left: 50, width: 50, height: 50}
        var editable = new $.fn.annotateEdit_rext(image, note);
...
// Line angle
        var lineAngle = Math.atan((this.note.y2 - this.note.y1) / (this.note.x2 - this.note.x1))
// Angle for arrow heads
        var end1 = lineAngle + 45 * Math.PI / 180
        var end2 = lineAngle - 45 * Math.PI / 180
// end points of arrow heads
        var y3 = this.note.y2 - arrowHeadLength * Math.sin(end1)
        var x3 = this.note.x2 - arrowHeadLength * Math.cos(end1)
        var y4 = this.note.y2 - arrowHeadLength * Math.sin(end2)
...
//        this.area.draggable('destroy');
    ...
    $.fn.annotateView.prototype.edit = function () {
        ...
        if (this.image.mode == 'view') {
            ...
            var editable = new $.fn.annotateEdit_rext(this.image, this.note);
